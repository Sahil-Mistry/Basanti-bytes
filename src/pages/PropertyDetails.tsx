...
const AMENITIES = [
  {
    cat: "Schools",
    count: 14,
    items: ["St. Andrews High", "Holy Family", "Jamnabai Narsee"],
  },
  {
    cat: "Hospitals",
    count: 7,
    items: ["Lilavati Hospital", "Holy Family Hospital", "Bandra Maternity"],
  },
  {
    cat: "Transport",
    count: 8,
    items: ["Bandra W Station", "Metro Line 2", "BEST Bus Depot"],
  },
];
...
export default function PropertyDetails() {
  ...
  const [localityData, setLocalityData] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const lat = searchParams.get("lat") ?? (property ? String(property.lat) : "");
  const lon = searchParams.get("lon") ?? (property ? String(property.lng) : "");
  const locality = searchParams.get("name") ?? property?.loc ?? "";

  useEffect(() => {
    if (!property || !lat || !lon || !locality) {
      return;
    }
    ...
    apiClient
      ...
      .catch(() => {
        ...
        setLocalityError("Unable to load locality details.");
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!isLocalityLoading && !localityError && localityData && (
          <pre
            style={{
              margin: 0,
              whiteSpace: "pre-wrap",
          ...
          >
            ...
            <div className="progress-track">
              <div
                ...
                style={{ width: `${score.val}%`, background: score.color }}
              />
            </div>
          </div>
        ))}
