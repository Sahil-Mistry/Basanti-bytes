...
const AREAS = [
  {
    ...
    lat: 23.0395,
    lng: 72.526,
    builderId: "69ec961a569e44de32952277",
  },
  {
    label: "Bandra West",
    lat: 19.0596,
...
const SCORE_META: Record<
  SubKey,
  { label: string; icon: string; backFn: (d: NeighborhoodData) => string }
> = {
  safety: {
    label: "Safety",
    icon: "🛡",
    backFn: (d) =>
      d.sources.safety.error
        ? "News sentiment analysis"
        : d.sources.safety.source,
  },
  aqi: {
    label: "Air Quality",
    icon: "🌿",
    backFn: (d) =>
      `AQI ${d.sources.aqi.value} · ${d.sources.aqi.stationName.split(",")[0]}`,
  },
  amenity: {
    label: "Amenities",
    icon: "🏪",
    backFn: (d) =>
      `${Object.values(d.sources.amenity.counts).reduce((a, b) => a + b, 0)} places nearby`,
  },
  connectivity: {
    label: "Connectivity",
    icon: "🚇",
    backFn: (d) =>
      `Airport ${d.sources.connectivity.driveTimes.airport ?? "—"} min drive`,
  },
  futureGrowth: {
    label: "Future Growth",
    icon: "📈",
    backFn: (d) =>
      `${d.sources.futureGrowth.nearestFeature} · ${d.sources.futureGrowth.expectedYear}`,
  },
  builder: {
    label: "Builder Trust",
    icon: "🏗",
    backFn: (d) =>
      `${d.sources.builder.name} · ${d.sources.builder.projectsDelivered} projects`,
  },
};
    ...
    >
      <div
        style={{
          ...
          inset: 0,
          background:
            "linear-gradient(90deg, transparent, rgba(206,191,172,.3), transparent)",
                      ...
                      <div className="card-3d">
                        ...
                        <div
                          ...
                          style={{
                            ...
                            border: "1px solid var(--c-border)",
                            padding: 16,
                            boxShadow: "0 2px 8px rgba(14,30,32,.06)",
                          }}
                        >
          ...
          {loading
            ...
            : data
              ? DRIVE_META.map((m) => {
                  ...
                  const isGood =
                    val !== undefined ? val <= m.goodThreshold : true;
                  return (
                    <div
                      key={m.key}
                      className="stat-3d"
                    ...
                    >
                      ...
                      <div
                        style={{
                          ...
                          fontWeight: 600,
                          marginTop: 4,
                          color: count > 0 ? "#0E5860" : "var(--c-text-sm)",
                        }}
                      >
          ...
          {loading ? (
            ...
            >
              <div>
                ...
                >
                  {data.sources.futureGrowth.distanceKm === 0
                    ? "Within locality"
              ...
              >
                ...
                <div
                  style={{
                    ...
                    fontSize: 24,
                    fontWeight: 500,
                    color: scoreColor(data.sources.builder.score),
                  }}
                >
                ...
                {[
                  {
                    ...
                    val: data.sources.builder.projectsDelivered,
                    unit: "",
                    good: true,
                  },
                  {
                    label: "Complaints Filed",
                    val: data.sources.builder.complaintsCount,
                    unit: "",
                    good: data.sources.builder.complaintsCount < 10,
                  },
                  {
                    label: "Avg Delay",
                    val: data.sources.builder.avgDelayMonths,
                    unit: " mo",
                    good: data.sources.builder.avgDelayMonths < 6,
                  },
                ].map((item, i, arr) => (
                  <div
                    key={item.label}
                    style={{
                      padding: "14px 20px",
                      borderRight:
                        i < arr.length - 1 ? "1px solid var(--c-bg)" : "none",
                    }}
                  >
                    <div
                      style={{
                    ...
                    >
                      {item.val}
                      <span style={{ fontSize: 11, color: "var(--c-text-sm)" }}>
                        {item.unit}
                      </span>
                    </div>
                    <div
